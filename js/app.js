...
function render(){
 ...
 <div class="card-body"><div class="why">${c.why}</div><div class="quick"><div class="box"><div class="label">Best-fit roles</div><div>${c.roles.slice(0,3).join(' · ')}</div></div><div class="box"><div class="label">Current evidence</div><div>${c.evidence}</div></div></div>
 ...
 day1Grid.innerHTML='';
 day1Top6.forEach(c=>{const el=document.createElement('article');el.className='card';el.innerHTML=`<div class="card-head"><div class="rank">${c.rank}</div><div><div class="company">${c.name}</div><div class="meta"><span class="pill">Day 1 · ${c.booth}</span><span class="pill">${c.geo}</span><span class="pill">${c.track}</span></div></div></div><div class="card-body"><div class="why">${c.why}</div><div class="quick"><div class="box"><div class="label">Roles to investigate</div><div>${c.roles.join(' · ')}</div></div><div class="box"><div class="label">Why research it</div><div>${c.evidence}</div></div></div><div class="actions"><a class="linkbtn" href="${c.careers}" target="_blank" rel="noopener">Careers ↗</a><a class="linkbtn" href="${c.research}" target="_blank" rel="noopener">Research / Evidence ↗</a></div><div class="details open"><div class="question"><div class="label">Question option 1</div>${c.question}</div><div class="question" style="margin-top:8px"><div class="label">Question option 2</div>${c.question2}</div></div></div>`;day1Grid.appendChild(el);});
 ...
 tier2Companies.filter(c=> matchesFilter(c,f) && (!q || JSON.stringify(c).toLowerCase().includes(q))).forEach(c=>{
  ...
  el.innerHTML=`<div class="backup-name">${c.name}</div><div class="backup-meta"><span class="pill">Day ${c.day} · ${c.booth}</span><span class="pill">${c.geo}</span><span class="pill">${c.track}</span></div><div class="backup-why">${c.why}</div><div class="backup-actions"><a class="linkbtn" href="${c.careers}" target="_blank" rel="noopener">Careers ↗</a></div>`;
